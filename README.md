# RMS Serious Alarm Monitor — Southaven

Watches the Geek+ RMS at **http://10.27.72.181** and posts to Slack **#hpy-southaven-auto-go-live**:
...
Created while running: `monitor-Southaven.log` (activity log), `state-Southaven.json` (alarms already alerted on), `map-latest.png` / `map-test.png`.

---

## First-time setup (on the Southaven hub PC)
...
3. **Invite the bot** to the Slack channel: in **#hpy-southaven-auto-go-live** type `/invite @<bot name>`.
...
6. **Check it started:** open `monitor-Southaven.log` — you should see `Starting monitor…` and `Logged in to RMS`.
...
Admin PowerShell:
```
Stop-ScheduledTask  -TaskName "RMS Serious Alarm Monitor - Southaven"
Start-ScheduledTask -TaskName "RMS Serious Alarm Monitor - Southaven"
Get-ScheduledTask   -TaskName "RMS Serious Alarm Monitor - Southaven" | Select-Object State
```
...
| `Slack.ChannelId` | Channel to post in (must be the ID) | C07NF8WGNAJ |
...
2. **Integrations** → add **Slack** → choose **#hpy-southaven-auto-go-live**.
3. **Add Check** → name it `Southaven RMS monitor` → **Period: 1 minute**, **Grace: 5 minutes** → make sure the Slack integration is ticked for this check.
...
Admin PowerShell:
```
Unregister-ScheduledTask -TaskName "RMS Serious Alarm Monitor - Southaven" -Confirm:$false
```
